...
class WidgetEngine {
  ...
  setWidget(id, customPath) {
    ...
    // Send to floating mini-widget via IPC
    try {
      if (window.windyAPI?.updateWidget) {
        if (customPath) {
          window.windyAPI.updateWidget({ type: 'custom', dataUrl: customPath });
        } else if (id === 'tornado') {
          window.windyAPI.updateWidget({ type: 'default' });
        } else {
          const stock = WidgetEngine.STOCK_WIDGETS[id];
          if (stock) {
            window.windyAPI.updateWidget({ type: 'stock', svg: stock.svg });
          }
        }
      }
    } catch (_) { }
