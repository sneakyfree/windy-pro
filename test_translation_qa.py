#!/usr/bin/env python3
"""
Translation QA Test Suite
Tests Windy Translate Spark and Translate Standard against base M2M-100 models
"""
...
def translate_text(text, model, tokenizer, tgt_lang):
...
def test_model_pair(model_config):
    ...
    for sentence_idx, sentence in enumerate(TEST_SENTENCES, 1):
        for lang_code, lang_name in LANGUAGE_PAIRS.items():
            ...
            try:
                # Get our translation
                our_translation = translate_text(sentence, our_model, our_tokenizer, lang_code)

                # Get base translation
                base_translation = translate_text(sentence, base_model, base_tokenizer, lang_code)
...
"""
    ...
    report += "Both translation models have been tested against their base M2M-100 counterparts. "
    report += "This completes the QA certification for all 16 core Windy Pro models.\n\n"
...
def main():
    """Main test execution"""
    print("="*80)
    print("TRANSLATION QA TEST SUITE")
    print("="*80)
    ...
    report = generate_markdown_report([r[1] for r in all_results])
